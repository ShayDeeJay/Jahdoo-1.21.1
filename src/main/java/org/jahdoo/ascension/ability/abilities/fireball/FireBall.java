...
public class FireBall extends DefaultEntityBehaviour {
    ...
    public void onEntityHit(LivingEntity hitEntity) {
        ...
        DamageUtils.damageWithJahdoo(hitEntity, this.elementProjectile.getOwner(), damage);
        onHitBehaviour();
    }
    ...
    private void fireballTrailingSound(){
        if (this.elementProjectile.tickCount % 7 == 0) {
            Helpers.getSoundWithPosition(this.elementProjectile.level(), this.elementProjectile.blockPosition(), SoundEvents.FIRE_AMBIENT, 1, 1.5f);
        }
    }
            ...
            livingEntity -> {
                if(canDamageEntity(livingEntity, (LivingEntity) this.elementProjectile.getOwner())){
                    if (!this.hitTargets.contains(livingEntity.getUUID())) {
                        DamageUtils.damageWithJahdoo(livingEntity, this.elementProjectile.getOwner(), this.damage);
    ...
    private void fireball(){
        var getPositions = Helpers.getRandomParticleVelocity(this.elementProjectile, 0.1);
        var getPositions2 = Helpers.getRandomParticleVelocity(this.elementProjectile, 0.05);
    ...
    private void onHitBehaviour() {
        ...
        Helpers.getSoundWithPosition(this.elementProjectile.level(), this.elementProjectile.blockPosition(), SoundRegister.EXPLOSION.get(),2f);
        Helpers.getSoundWithPosition(this.elementProjectile.level(), this.elementProjectile.blockPosition(), SoundEvents.FIRE_AMBIENT);
