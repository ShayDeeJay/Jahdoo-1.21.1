...
public class ChallengeAltarRenderer extends GeoBlockRenderer<ChallengeAltarBlockEntity>{
    public static final ResourceLocation BEAM_LOCATION = Helpers.res("textures/entity/beacon_beam.png");
    ...
    private static void activationNova(PoseStack poseStack, ChallengeAltarBlockEntity animatable, MultiBufferSource bufferSource, boolean isActivated) {
        if(!isActivated){
            if(animatable.privateTicks < 120){
                ...
                drawTexture(poseStack.last(), bufferSource, 255, (float) animatable.animateTick, Helpers.res("textures/entity/shield.png"), FastColor.ARGB32.color(Math.max(0, (50 - (animatable.privateTicks - 94) * 2)), ColourStore.PERK_GREEN));
                poseStack.popPose();
    ...
    private static void activeBeam(PoseStack poseStack, ChallengeAltarBlockEntity animatable, MultiBufferSource bufferSource, float partialTick, long i, boolean isActivated) {
        ...
        var colourLight = Helpers.getColourLight(ColourStore.PERK_GREEN, Math.min(Math.max((double) animatable.privateTicks / 40, 0.5), 1.2));
